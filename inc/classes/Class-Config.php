...
final class fitness {
	...
	private function setup() {
		...
		$enqueu = new Fitness_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->fitness_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'fitness_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'fitness', FITNESS_DIR_PATH . '/languages' );
		...
		add_image_size( 'fitness_widget_post_thumb', 100, 60, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'fitness-np-thumb', 60, 60, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'fitness' ),
            'social-menu'   => esc_html__( 'Social Menu', 'fitness' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = FITNESS_DIR_CSS_URI;
		$jsPath  = FITNESS_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					'handler'		=> 'fitness-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->fitness_version,
				),
				array(
					'handler'		=> 'fitness-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'fitness' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate fitness theme customizer
		$fitness_theme_customizer = new fitness_theme_customizer();
	}
	...
	public function set_elementor_flag() {
		update_option( 'fitness_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'fitness-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'fitness_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'fitness-elementor-default-style',  FITNESS_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->fitness_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'fitness_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'fitness-elementor-notice', FITNESS_DIR_JS_URI.'fitness-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'fitness-elementor-notice',
				'bakeryElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'fitness-elementor-notice-nonce' ),
				)
			);
		}
