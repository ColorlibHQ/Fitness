...
class fitness_author_widget extends WP_Widget {
...
'fitness_author_widget',


// Widget name will appear in UI
esc_html__( '[ Fitness ] Author Widget', 'fitness' ),

// Widget description
array( 'description' => esc_html__( 'Add author content', 'fitness' ), )
);
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Author Name', 'fitness' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'designation' ) ); ?>"><?php esc_html_e( 'Designation:' ,'fitness'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'textarea' ) ); ?>"><?php esc_html_e( 'Short Descriptions:' ,'fitness'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Facebook Url' ) ); ?>"><?php esc_html_e( 'Facebook Url:' ,'fitness'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Twitter Url' ) ); ?>"><?php esc_html_e( 'Twitter Url:' ,'fitness'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Dribbble Url' ) ); ?>"><?php esc_html_e( 'Dribbble Url:' ,'fitness'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Behance Url' ) ); ?>"><?php esc_html_e( 'Behance Url:' ,'fitness'); ?></label>
...
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>"><?php _e( 'Image', 'fitness' ); ?>:</label>
	<div class="fitness-media-container">
		<div class="fitness-media-inner">
			...
			<span class="fitness-no-image" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-noimg" <?php echo wp_kses_post( $no_img_style ); ?>><?php _e( 'No image selected', 'fitness' ); ?></span>
		</div>
	
	<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" value="<?php echo esc_attr( $image ); ?>" class="fitness-media-url" />

	<input type="button" value="<?php echo _e( 'Remove', 'fitness' ); ?>" class="button fitness-media-remove" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-remove" <?php echo wp_kses_post( $img_style ); ?> />

	<?php $button_text = ( $image != '' ) ? __( 'Change Image', 'fitness' ) : __( 'Select Image', 'fitness' ); ?>
	<input type="button" value="<?php echo esc_html( $button_text ); ?>" class="button fitness-media-upload" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-button" />
	<br class="clear">
	</div>
</p>
<style>
.fitness-media-container {
	width: 98%;
}
...
.fitness-media-remove {
	float: left;
	width: 48%;
}

.fitness-media-upload {
	float: right;
	width: 48%;
}
</style>
...
jQuery(function($){
    ...
    wpshed_image_upload( '.fitness-media-upload' );
    ...
    wpshed_image_remove( '.fitness-media-remove' );
...
} // Class fitness_author_widget ends here


// Register and load the widget
function fitness_author_load_widget() {
	register_widget( 'fitness_author_widget' );
}
add_action( 'widgets_init', 'fitness_author_load_widget' );