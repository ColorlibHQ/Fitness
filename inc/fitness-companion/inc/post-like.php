...
add_action( 'wp_enqueue_scripts', 'fitness_sl_enqueue_scripts' );
function fitness_sl_enqueue_scripts() {

	wp_enqueue_script( 'fitness-post-likes', FITNESS_COMPANION_DIR_URL . '/js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'fitness-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'fitness' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_fitness_process_simple_like', 'fitness_process_simple_like' );
add_action( 'wp_ajax_fitness_process_simple_like', 'fitness_process_simple_like' );
function fitness_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'fitness' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'fitness' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'fitness' );
		$icon = $icon_empty;
	}
...
// User Profile List
add_action( 'show_user_profile', 'fitness_show_user_likes' );
add_action( 'edit_user_profile', 'fitness_show_user_likes' );
function fitness_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'fitness' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'fitness' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
