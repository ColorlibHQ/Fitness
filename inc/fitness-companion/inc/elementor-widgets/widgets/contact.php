 ...
 * @since 1.0
 */
class Fitness_Contact extends Widget_Base {

    public function get_name() {
        return 'fitness-contact';
    }

    public function get_title() {
        return __( 'Contact', 'fitness' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'contact_map',
            [
                'label' => __( 'Map Settings', 'fitness' ),
            ]
        );
        $this->add_control(
            'map_switch',
            [
                'label' => __( 'Show Map', 'fitness' ),
        ...
        $this->add_control(
            'map_address',
            [
                'label'         => esc_html__( 'Address', 'fitness' ),
                'type'          => Controls_Manager::TEXT,
                'label_block'   => true,
                'default'       => esc_html__( 'Rabindra Sorobor Dhanmondi Lake Rd', 'fitness' ),
                'description'   => esc_html__( 'Before use map widget make sure you are set google map api key from customizer -> theme options -> general -> google map api key', 'fitness' )
            ]
        );
        $this->add_control(
            'map_lat',
            [
                'label'       => esc_html__( 'Latitude', 'fitness' ),
                'label_block' => true,
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( '23.745227', 'fitness' )
            ]
        );
        $this->add_control(
            'map_lng',
            [
                'label'       => esc_html__( 'Longitude', 'fitness' ),
                'label_block' => true,
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( '90.377155', 'fitness' )
            ]
        );
        ...
        $this->start_controls_section(
            'contact_info',
            [
                'label' => __( 'Contact Info', 'fitness' ),
            ]
        );

        $this->add_control(
            'info', [
                'label'         => __( 'Create Contact Info', 'fitness' ),
                ...
                'fields'  => [
                    [
                        ...
                        'label_block' => true,
                        'type'        => Controls_Manager::TEXT,
                        'default'     => esc_html__( 'Dhaka, Bangladesh', 'fitness' )
                    ],
                    [
                        'name'    => 'desc',
                        'label'   => __( 'Contact Descriptions', 'fitness' ),
                        'type'    => Controls_Manager::WYSIWYG,
                        'default' => esc_html__( 'Write something...', 'fitness' )
                    ],
                    [
                        'name'  => 'icon',
                        'label' => __( 'Icon', 'fitness' ),
        ...
        $this->add_control(
            'color_title', [
                'label'     => __( 'Address Title Color', 'fitness' ),
        ...
        $this->add_control(
            'color_desc', [
                'label'     => __( 'Address Description Color', 'fitness' ),
                'type'      => Controls_Manager::COLOR,
                'default'   => '#777',
                'selectors' => [
    ...
    wp_enqueue_script('fitness-map-custom');
                    ...
                    ?>
                        <div class="single-contact-address d-flex flex-row">
                            ...
                            <div class="contact-details">
                                ...
                                //
                                if( ! empty( $info['desc'] ) ) {
                                    echo fitness_get_textareahtml_output( $info['desc'] );
                                }
                                ?>
                            </div>
                        </div>
